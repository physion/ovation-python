...
@istest
def should_get_requisition():
    s = Mock(spec=session.Session)
    sv3 = Mock(spec=session.Session)
    s.with_prefix.return_value = sv3
    sv3.path.return_value = sentinel.requisition_path
    sv3.get.return_value = session.simplify_response({})

    requisitions.get_requisition(s, sentinel.req_id)

    s.with_prefix.assert_called_with('/api/v3')
    sv3.path.assert_called_with('requisitions', id=sentinel.req_id, include_org=False)
    sv3.get.assert_called_with(sentinel.requisition_path)
